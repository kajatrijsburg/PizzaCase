...
namespace PizzaCase
{
    internal static class Encryption
    {
        ...
        public static string Encrypt(string plainText, string key, string InitalizationVector)
        {
            ...
            return Encoding.Unicode.GetString(encrypted_bytes);
        }

        /// <summary>
        /// Takes plain text, a key and an initialization vector and encrypts it, returning the result as a string.
        /// </summary>
        /// <param name="plainData"> the data that will be encrypted</param>
        /// <param name="key">the encryption key</param>
        /// <param name="InitalizationVector">sets the initialization vector. This number is needed by the encryption algorithm to correctly encrypt the data.</param>
        /// <returns>encrypted data</returns>
        /// <exception cref="ArgumentNullException"> throws on null imput</exception>
        public static byte[] Encrypt_bytes(byte[] plainData, string key, string InitalizationVector)
        {
            // check arguments
            {
            ...
            //encrypt
            return aes.EncryptCbc(plainData, aes.IV);
        }
        ...
        public static string Decrypt(string encryptedString, string key, string InitalizationVector) {
            ...
            // convert arguments to byte arrays
            byte[] encrypted_bytes = Encoding.Unicode.GetBytes(encryptedString);
            byte[] key_bytes = Encoding.Unicode.GetBytes(key);
            byte[] iv_bytes = Encoding.Unicode.GetBytes(InitalizationVector);
        ...
        /// takes an encrypted string, key and an initialization vector and decrypts the string, returning the plain text
        /// </summary>
        /// <param name="encryptedData"> the byte array that will be decrypted</param>
        /// <param name="key">the encryption key</param>
        /// <param name="InitalizationVector">sets the initialization vector. This number is needed by the encryption algorithm to correctly encrypt the data.</param>
        /// <returns>encrypted data</returns>
        /// <exception cref="ArgumentNullException"> throws on null imput</exception>
        public static byte[] Decrypt_bytes(byte[] encryptedData, string key, string InitalizationVector)
        {
            // check arguments
            {
            ...
            // convert arguments to byte arrays
            byte[] encrypted_bytes = encryptedData;
            byte[] key_bytes = Encoding.Unicode.GetBytes(key);
            byte[] iv_bytes = Encoding.Unicode.GetBytes(InitalizationVector);
            ...
            //create decryption stream
            return aes.DecryptCbc(encrypted_bytes, aes.IV);
        }
    }
}
